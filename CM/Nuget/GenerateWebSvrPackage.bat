...
echo ^<id^>Modulyn.Server^</id^> >> %nuspecFile%
...
for %%i in ("%nugetDir%\data\websvr\dependencies*.xml") do (
    type %%i >> %nuspecFile%
	echo. >> %nuspecFile%
)
echo ^</dependencies^> >> %nuspecFile%
...
echo ^<file src="%sourceDir%\ModulynServer\**" target="content" /^> >> %nuspecFile%
echo ^<file src="%nugetdir%\Data\websvr\ReadMe.md" target="docs\" /^> >> %nuspecFile%
